...
class StartGameMapCommand(MapCommand):
    ...
    def __init__(self, map:Map, args:tuple[str]):
        super().__init__(self, map, args)

    
    def execute(self) -> bool:
        super().get_map().get_game_flow_director().next_state()
        
    
    def check(self) -> bool:
        if not isinstance(super().get_map().get_game_flow_director().get_state(), StartState):
