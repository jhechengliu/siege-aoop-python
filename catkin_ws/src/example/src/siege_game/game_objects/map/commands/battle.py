...
class PlayerCheckSightBattleCommand(MapCommand):
    ...
    def execute(self) -> str:
        ...
        return "success_" + json.dumps(msg).replace(" ", "") #convert from dictionary to json-formatted string
...
class ReadyBattleBattleCommand(MapCommand):
    ...
    def execute(self) -> None:
        ...
        if (self.get_map().get_attacker_ready_battle() and self.get_map().get_defender_ready_battle()):
            if (self.__game.get_client_A_player().get_identity() == Identity.ATTACK):
